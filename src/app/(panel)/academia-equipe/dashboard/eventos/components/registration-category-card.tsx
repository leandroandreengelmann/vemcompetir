...
export function RegistrationCategoryCard({ eventId, category, onClick, onAddToCart, isSelected = false, isWhiteBelt = false, isInCart = false }: CategoryCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className={`group flex flex-col p-4 rounded-3xl border ${isActuallyInCart ? 'border-green-500 bg-green-50/30 dark:bg-green-500/10' : (isSelected ? 'border-primary ring-2 ring-primary ring-offset-2 bg-primary/5' : 'border-border bg-card hover:shadow-md hover:border-primary/20')} shadow-sm transition-all cursor-pointer focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background active:scale-[0.99] outline-none overflow-hidden`}
        >
