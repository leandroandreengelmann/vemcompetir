...
import { ShoppingCartIcon, CheckIcon, MagnifyingGlassIcon, StudentIcon } from '@phosphor-icons/react';
...
function fakeFee(beltKey: string): number {
    return beltKey === 'roxa' ? 180 : beltKey === 'azul' ? 170 : 160;
}

export function AamepJuvenilAthleteView() {
    ...
    const allCategories = useMemo(() => {
        ...
        for (const belt of AAMEP_JUVENIL_BELTS) {
            for (const w of WEIGHTS) {
                ...
                list.push({
                    id: `aamep-juvenil-${belt.key}-${w.key}`,
                    beltKey: belt.key,
                    beltLabel: belt.label,
                    weightKey: w.key,
                    weightName: w.name,
                    range,
                    categoryName: `Juvenil • ${generoLabel} • ${belt.label} • ${w.name} (${range}) • Kimono`,
        ...
        <div className="space-y-5">
            <Card className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
                <CardContent className="py-4 flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <h3 className="font-bold leading-tight">AAMEP Juvenil Demo 2026</h3>
                        <p className="text-xs text-muted-foreground">Federação AAMEP · {AAMEP_JUVENIL.ageRange} · {AAMEP_JUVENIL.fightTime} de luta</p>
                    </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="text-xs text-muted-foreground">
                    Juvenil · {generoLabel} · Gi · {filtered.length} visíveis
                </div>
            </div>
