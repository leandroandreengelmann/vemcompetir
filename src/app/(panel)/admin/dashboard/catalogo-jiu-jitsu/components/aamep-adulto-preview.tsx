...
export function AamepAdultoPreview() {
    ...
    const visibleBelts = useMemo(
        () => (activeBelt === 'all' ? AAMEP_ADULTO_BELTS : AAMEP_ADULTO_BELTS.filter((b) => b.key === activeBelt)),
        [activeBelt],
    );

    const total = AAMEP_ADULTO_BELTS.length * WEIGHTS.length;
        ...
        <div className="space-y-6">
            <Card>
                <CardHeader className="pb-3">
                    <CardTitle className="text-panel-md font-semibold">Os 5 eixos · AAMEP Adulto</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="grid gap-3 md:grid-cols-5">
                        <AxisChip label="Faixa Etária" value="Adulto" sub={AAMEP_ADULTO.ageRange} />
                        ...
                        <AxisChip label="Modalidade" value="Gi (com kimono)" sub={`Tempo: ${AAMEP_ADULTO.fightTime}`} />
                    </div>
                    <div className="mt-4 text-sm text-muted-foreground">
                        Combinando os eixos:{' '}
                        <span className="font-semibold text-foreground">
                            {AAMEP_ADULTO_BELTS.length} × {WEIGHTS.length} = {total} categorias por gênero
                        </span>
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b bg-muted/30">
                    <div className="flex items-center gap-3">
                        <span className="text-base font-semibold">
                            Adulto <span className="text-muted-foreground font-normal">· {AAMEP_ADULTO.ageRange}</span>
                        </span>
                    ...
                    <span className="text-xs text-muted-foreground hidden md:inline">
                        {generoLabel} · Gi · {AAMEP_ADULTO.fightTime}
                    </span>
                </div>
            ...
            <Card className="bg-muted/30 border-dashed">
                <CardContent className="py-4 text-sm text-muted-foreground space-y-2">
                    <p>
                        <strong className="text-foreground">AAMEP só Gi:</strong> a federação não modela No-Gi nas tabelas oficiais.
                        Peso é com kimono.
                    </p>
                    <p>
                        <strong className="text-foreground">Pesos masculinos:</strong> 9 (Galo até Pesadíssimo).{' '}
                        <strong className="text-foreground">Femininos:</strong> 8 (Galo até Super Pesado livre — não tem Pesadíssimo).
                        Absoluto fica em tabela própria.
                    </p>
                </CardContent>
            </Card>
        </div>
