...
import { ShoppingCartIcon, CheckIcon, MagnifyingGlassIcon, TrophyIcon } from '@phosphor-icons/react';
...
function fakeFee(beltKey: string): number {
    const base = beltKey === 'preta' ? 240 : beltKey === 'marrom' ? 220 : beltKey === 'roxa' ? 210 : beltKey === 'azul' ? 200 : 190;
    return base;
}

export function AamepMasterAthleteView() {
        ...
        for (const age of AAMEP_MASTER_AGES) {
            for (const belt of AAMEP_MASTER_BELTS) {
                for (const w of age.weights) {
                    ...
                    list.push({
                        id: `aamep-master-${age.key}-${belt.key}-${w.key}`,
                        ageKey: age.key,
                        ageLabel: age.label,
                        ageRange: age.range,
                        beltKey: belt.key,
                        ...
                        weightName: w.name,
                        range,
                        categoryName: `${age.label} • Masculino • ${belt.label} • ${w.name} (${range}) • Kimono`,
        ...
        <div className="space-y-5">
            <Card className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
                <CardContent className="py-4 flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <h3 className="font-bold leading-tight">AAMEP Master Demo 2026</h3>
                        <p className="text-xs text-muted-foreground">Federação AAMEP · {AAMEP_MASTER.ageRange} · {AAMEP_MASTER.fightTime} de luta · Apenas Masculino</p>
                    </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-xs text-muted-foreground">
                    Master · Masculino · Gi · {filtered.length} visíveis
                </div>
            </div>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    placeholder="Buscar pela categoria... (ex: pluma, médio, master 1, azul)"
