...
export default async function AthleteCampeonatos() {
    ...
    return (
        <div
            className="min-h-screen bg-[#FAFAFA] p-4 flex flex-col gap-6"
            style={{
        ...
        >
            ...
            {/* Search Bar (Placeholder for local filtering if needed) */}
            ...
            <main className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {!events || events.length === 0 ? (
                    ...
                    events.map(event => (
                        <Link key={event.id} href={`/atleta/dashboard/campeonatos/${event.id}`}>
                            <Card className="overflow-hidden border-none shadow-premium hover:shadow-xl transition-all active:scale-[0.98] group bg-white rounded-sm p-0 gap-0">
                                ...
                                <CardContent className="p-5 space-y-3">
                                    <h3 className={`font-bold text-h2 leading-tight transition-colors line-clamp-2 ${(profile as any)?.belt_color?.toLowerCase() === 'branca' ? 'text-brand-950' : 'text-primary'}`}>
                                        {event.title}
                                    </h3>
